...
/*
 * corrected
 * By Saint Father (utf-8 encoding)
 * 20 May 2010
 */
...
/**
 * Функция склонения строки в соответствии с числом <b>value</b><br/>
 * В функцию необходимо передать три варианта склонения, например
 * <code><pre>
 * rusNumber(23, 'кошка', 'кошки', 'Кошек'); // = кошки 
 * rusNumber(13, 'кошка', 'кошки', 'Кошек'); // = Кошек
 * </pre></code> 
 * @param {Number} value
 * @param {String} w1 «Объект»
 * @param {String} w2 «Объекта»
 * @param {String} w5 «Объектов»
 * @return {String} Один из трех вариантов написания
 */
rusNumber = function(value, w1, w2, w5) {
	var vv = value % 100;
	var v = value % 10;
	if (vv > 10 && vv < 20) return w5;
	if (v > 1 && v < 5) return w2;
	if (v == 1) return w1;
	return w5;
}

if(Ext.grid.GridPanel){
	Ext.grid.GridPanel.prototype.ddText = "{0} {1}";
	Ext.grid.GridPanel.prototype.getDragDropText = function(){
		var count = this.selModel.getCount();
		return String.format(this.ddText, count,
			rusNumber(count,
				'выбранная строка',
				'выбранные строки',
				'выбранных строк'
...
if(Ext.DatePicker){
  Ext.apply(Ext.DatePicker.prototype, {
    ...
    nextText           : 'Следующий месяц (Ctrl+Вправо)',
    prevText           : 'Предыдущий месяц (Ctrl+Влево)',
    monthYearText      : 'Выбор месяца (Ctrl+Вверх/Вниз для выбора года)',
    todayTip           : "{0} (Пробел)",
    format             : "d.m.y",
    okText             : "&#160;OK&#160;",
    cancelText         : "Отмена",
...
if(Ext.form.DateField){
  Ext.apply(Ext.form.DateField.prototype, {
    ...
    invalidText       : "{0} не являетсяя правильной датой - дата должна быть указана в формате {1}",
    format            : "d.m.y",
